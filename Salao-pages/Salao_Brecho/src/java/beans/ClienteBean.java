...
public class ClienteBean {
    ...
    public void inserir() {
        ClienteDAO dao = new ClienteDAO();
        cliente.setEndereco("N/D");
        cliente.setSexo("N/D");
        cliente.setMaracao(new Date());
        String aviso= String.join(" ", cliente.getNome(), cliente.getApelido()).concat(" Tem a marcação registada");
        try {
          dao.inserirCliente(cliente);
            
        } catch (Exception e) {
            aviso = e.getLocalizedMessage();
        }
        addMessage(aviso);
        cliente = new Cliente();
    }
